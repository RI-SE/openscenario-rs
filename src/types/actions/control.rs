...
///
/// `Default` is a `#[derive]`: every attribute is optional
...
pub struct AutomaticGear {
    #[serde(rename = "@gear")]
    pub gear: Value<AutomaticGearType>,
}

// `AutomaticGearType` used to be declared a second time right here, with its own
// `#[serde(rename)]` transcription of the same variant -> wire-name table that
// `src/types/enums.rs` already carries. The two agreed (`p`/`r`/`n`/`d` both times),
// but only the `enums.rs` copy is generated by `osc_enum!` and therefore has the
// `Display`/`FromStr` that `Value<T>` serializes and deserializes through -- and only
// that copy is covered by `tests/enum_wire_names_test.rs`. A duplicate that nothing
// pins is exactly the drift `osc_enum!` closed structurally, so the local copy is gone and
// this path re-exports the canonical one instead.
pub use crate::types::enums::AutomaticGearType;
...
impl AutomaticGear {
    ...
    pub fn park() -> Self {
        Self {
            gear: Value::Literal(AutomaticGearType::Park),
        }
    }
    ...
    pub fn reverse() -> Self {
        Self {
            gear: Value::Literal(AutomaticGearType::Reverse),
        }
    }
    ...
    pub fn neutral() -> Self {
        Self {
            gear: Value::Literal(AutomaticGearType::Neutral),
        }
    }
...
impl Gear {
    ...
    pub fn automatic(gear_type: AutomaticGearType) -> Self {
        Self::AutomaticGear(AutomaticGear {
            gear: Value::Literal(gear_type),
        })
    }
...
mod tests {
    ...
    fn test_automatic_gear_creation() {
        ...
        assert_eq!(park.gear, Value::Literal(AutomaticGearType::Park));

        let drive = AutomaticGear::drive();
        assert_eq!(drive.gear, Value::Literal(AutomaticGearType::Drive));

        let reverse = AutomaticGear::reverse();
        assert_eq!(reverse.gear, Value::Literal(AutomaticGearType::Reverse));
    ...
    fn test_gear_group_creation() {
        ...
        if let Gear::AutomaticGear(gear) = auto_gear {
            assert_eq!(gear.gear, Value::Literal(AutomaticGearType::Park));
        } else {
