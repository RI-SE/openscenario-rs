...
use openscenario_rs::types::{
    ...
    positions::{
        ...
        world::GeographicPosition, Position,
    },
};
...
fn test_relative_object_position_ahead() {
    ...
    assert_eq!(
        position.entity_ref,
        OSString::literal("follower".to_string())
    );
...
fn test_advanced_positions_serialization() {
    ...
    let trajectory_deserialized: TrajectoryPosition =
        serde_json::from_str(&trajectory_serialized).unwrap();
    assert_eq!(trajectory_pos, trajectory_deserialized);

    let geographic_serialized = serde_json::to_string(&geographic_pos).unwrap();
    let geographic_deserialized: GeographicPosition =
        serde_json::from_str(&geographic_serialized).unwrap();
    assert_eq!(geographic_pos, geographic_deserialized);

    let relative_serialized = serde_json::to_string(&relative_pos).unwrap();
    let relative_deserialized: RelativeObjectPosition =
        serde_json::from_str(&relative_serialized).unwrap();
