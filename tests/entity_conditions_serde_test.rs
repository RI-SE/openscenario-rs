    ...
    T: serde::Serialize + for<'de> serde::Deserialize<'de> + std::fmt::Debug,
{
    ...
    quick_xml::de::from_str(&xml).unwrap_or_else(|e| {
        panic!(
            "deserialize failed for {}: {e}\nXML was: {xml}",
            std::any::type_name::<T>()
        )
    })
}
...
fn test_offroad_condition_raw_xml_deserialize() {
    ...
    let condition: OffroadCondition =
        quick_xml::de::from_str(xml).expect("failed to deserialise OffroadCondition from raw XML");
...
fn test_angle_condition_serializes_correct_attribute_names() {
    ...
    let xml = quick_xml::se::to_string(&condition).expect("serialize failed");
    assert!(
        xml.contains("angleType="),
        "Expected angleType attribute, got: {xml}"
    );
    assert!(
        xml.contains("angleTolerance="),
        "Expected angleTolerance attribute, got: {xml}"
    );
    assert!(
        xml.contains("coordinateSystem="),
        "Expected coordinateSystem attribute, got: {xml}"
    );
    // Old snake_case names must not appear
    assert!(
        !xml.contains("angle_type="),
        "Old 'angle_type' must not appear, got: {xml}"
    );
    assert!(
        !xml.contains("angle_tolerance="),
        "Old 'angle_tolerance' must not appear, got: {xml}"
    );
    assert!(
        !xml.contains("coordinate_system="),
        "Old 'coordinate_system' must not appear, got: {xml}"
    );
}
...
fn test_relative_speed_condition_serializes_correct_attribute_names() {
    ...
    assert!(
        xml.contains("entityRef="),
        "Expected entityRef attribute, got: {xml}"
    );
    assert!(xml.contains("rule="), "Expected rule attribute, got: {xml}");
    assert!(
        xml.contains("value="),
        "Expected value attribute, got: {xml}"
    );
    // Old snake_case field names must not appear
    assert!(
        !xml.contains("entity_ref="),
        "Old 'entity_ref' must not appear, got: {xml}"
    );
}
...
fn test_relative_lane_range_serializes_correct_attribute_names() {
    ...
    assert!(
        xml.contains("from=\""),
        "Expected from attribute, got: {xml}"
    );
...
fn test_relative_clearance_condition_raw_xml_deserialize() {
    ...
    assert_eq!(
        condition.relative_lane_ranges[0].from,
        Some(Int::literal(-1))
    );
...
fn test_relative_clearance_condition_serializes_correct_attribute_names() {
    ...
    let xml = quick_xml::se::to_string(&condition).expect("serialize failed");
    assert!(
        xml.contains("oppositeLanes="),
        "Expected oppositeLanes attribute, got: {xml}"
    );
    assert!(
        xml.contains("freeSpace="),
        "Expected freeSpace attribute, got: {xml}"
    );
    assert!(
        xml.contains("distanceForward="),
        "Expected distanceForward attribute, got: {xml}"
    );
    // Old snake_case names must not appear
    assert!(
        !xml.contains("opposite_lanes="),
        "Old 'opposite_lanes' must not appear, got: {xml}"
    );
    assert!(
        !xml.contains("free_space="),
        "Old 'free_space' must not appear, got: {xml}"
    );
    assert!(
        !xml.contains("distance_forward="),
        "Old 'distance_forward' must not appear, got: {xml}"
    );
}
