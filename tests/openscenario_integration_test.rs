...
mod cut_in_scenario_tests {
    ...
    fn can_access_cut_in_storyboard() {
        ...
        let scenario = parse_str(&xml).expect("cut_in_101_exam.xosc must parse successfully");
        {
            ...
            assert!(
                !init.actions.global_actions.is_empty() || !init.actions.private_actions.is_empty(),
    ...
    fn can_validate_cut_in_story_structure() {
        ...
        assert!(
            !storyboard.stories.is_empty(),
            "Storyboard must contain stories"
        );
        ...
        assert!(
            !act.maneuver_groups.is_empty(),
            "Act must contain maneuver groups"
        );
    ...
    fn can_roundtrip_cut_in_scenario() {
        ...
        let scenario =
            parse_str(&original_xml).expect("cut_in_101_exam.xosc must parse successfully");
        ...
        let scenario_entities = scenario
            .entities
            .as_ref()
            .expect("Scenario should have entities");
        ...
        let roundtrip_names: std::collections::HashSet<_> = roundtrip_entities
            .scenario_objects
            .iter()
            .filter_map(|obj| obj.get_name())
            .collect();
    ...
    fn can_parse_trajectory_system_fully() {
        ...
        assert!(
            !group.maneuvers.is_empty(),
            "ManeuverGroup must contain maneuvers"
        );
        ...
        println!(
            "   📖 Story: {}",
            story.name.as_literal().unwrap_or(&"Unknown".to_string())
        );
    ...
    fn can_access_trajectory_vertices_with_scientific_notation() {
        ...
        assert!(
            vertex_count > 0,
            "Should have trajectory vertices parsed successfully"
        );

        println!(
            "✅ Scientific notation trajectory vertices: {} estimated parsed",
            vertex_count
        );
    }
            ...
            if let Some(routing) = &action_wrapper.routing_action {
                if let Some(follow_action) = &routing.follow_trajectory_action {
                    ...
                    if let Some(trajectory) = &follow_action.trajectory {
                        ...
                        println!(
                            "  - Trajectory closed: {}",
                            trajectory.closed.as_literal().unwrap_or(&false)
                        );
                        ...
                        if let Some(polyline) = &trajectory.shape.polyline {
                            ...
                            assert!(
                                ...
                                "Polyline must contain vertices"
                            );
                        } else {
...
fn can_validate_trajectory_following_modes() {
    ...
    assert!(
        routing_actions_found > 0,
        "Should find RoutingActions in the scenario"
    );
    println!(
        "✅ Trajectory following modes validation: {} actions validated",
        routing_actions_found
    );
}
...
fn tdd_can_parse_story_level_start_triggers() {
    ...
    assert!(
        event.start_trigger.is_some(),
        "Events should have StartTrigger parsed"
    );

    let trigger = event.start_trigger.as_ref().unwrap();
    assert!(
        !trigger.condition_groups.is_empty(),
        "Trigger should contain ConditionGroups"
    );
...
fn can_create_complete_scenario_structure_with_story_hierarchy() {
    ...
    assert_eq!(
        maneuver_group.name.as_literal().unwrap(),
        "MainManeuverGroup"
    );
    ...
    println!(
        "   📚 Story '{}' with parameter declarations",
        story.name.as_literal().unwrap()
    );
    println!(
        "   🎬 Act '{}' with maneuver groups",
        act.name.as_literal().unwrap()
    );
    println!(
        "   👥 ManeuverGroup '{}' with actors and maneuvers",
        maneuver_group.name.as_literal().unwrap()
    );
    println!(
        "   🎯 Maneuver '{}' with events",
        maneuver.name.as_literal().unwrap()
    );
    println!(
        "   ⚡ Event '{}' with priority and triggers",
        event.name.as_literal().unwrap()
    );
...
fn test_parameter_declarations_with_constraints() {
    use openscenario_rs::types::{
        basic::{
            ParameterDeclaration, ParameterDeclarations, ValueConstraint, ValueConstraintGroup,
        },
        enums::ParameterType,
    };
    ...
    assert_eq!(
        constraints.value_constraints[1].value.as_literal().unwrap(),
        "200.0"
    );
...
<OpenSCENARIO>
  ...
  <Storyboard>
    ...
    if let Ok(scenario) = result {
        ...
        println!(
            "   📊 {} global actions, {} private actions",
