...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let duration = start.elapsed();
    println!(
        "✅ Created 100 complex scenarios (10 entities each) in {:?}",
        duration
    );
    ...
    println!(
        "✅ Created and stored {} scenarios in memory",
        scenarios.len()
    );
    if !scenarios.is_empty() {
        println!(
            "   Estimated memory per scenario: ~{} bytes",
    ...
    println!("=====================");
    println!(
        "• Simple scenario creation: ~{:?} per scenario",
        duration / 1000
    );
