...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        ...
        bbox.contains_point_with_params(0.0, 0.0, 0.0, &params)?
    );
    println!(
        "   - Distance to point (3,0,0): {:.2} m",
        bbox.distance_to_point(3.0, 0.0, 0.0, &params)?
    );
    ...
    println!(
        "   - Car footprint: {:.1} m²",
        car_dims.footprint_area(&params)?
    );
    println!(
        "   - Truck footprint: {:.1} m²",
        truck_dims.footprint_area(&params)?
    );
    println!(
        "   - Motorcycle footprint: {:.1} m²",
        motorcycle_dims.footprint_area(&params)?
    );
    println!(
        "   - Pedestrian footprint: {:.1} m²",
        pedestrian_dims.footprint_area(&params)?
    );
    ...
    println!(
        "   - Original car footprint: {:.1} m²",
        car_dims.footprint_area(&params)?
    );
    println!(
        "   - 1.5x scaled car footprint: {:.1} m²",
        scaled_car.footprint_area(&params)?
    );
    ...
    for (x, y, z) in test_points {
        ...
        println!(
            "   Point ({:.1}, {:.1}, {:.1}): contains={}, distance={:.2}m",
